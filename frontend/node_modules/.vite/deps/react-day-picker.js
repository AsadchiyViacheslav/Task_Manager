...
function rangeIncludesDate(range, date, excludeEnds = false, dateLib2 = defaultDateLib) {
  let { from, to } = range;
  const { differenceInCalendarDays, isSameDay } = dateLib2;
  if (from && to) {
...
function dateMatchModifiers(date, matchers, dateLib2 = defaultDateLib) {
  ...
  const { isSameDay, differenceInCalendarDays, isAfter } = dateLib2;
...
function createGetModifiers(days, props, navStart, navEnd, dateLib2) {
  ...
  const { isSameDay, isSameMonth, startOfMonth, isBefore, endOfMonth, isAfter } = dateLib2;
...
function getMonthOptions(displayMonth, navStart, navEnd, formatters, dateLib2) {
  const { startOfMonth, startOfYear, endOfYear, eachMonthOfInterval, getMonth } = dateLib2;
...
function getYearOptions(navStart, navEnd, formatters, dateLib2, reverse = false) {
  ...
  if (!navEnd)
    return void 0;
  const { startOfYear, endOfYear, eachYearOfInterval, getYear } = dateLib2;
...
function useAnimation(rootElRef, enabled, { classNames, months, focused, dateLib: dateLib2 }) {
  ...
  (0, import_react28.useLayoutEffect)(() => {
    ...
    const isSameMonth = dateLib2.isSameMonth(months[0].date, previousMonths[0].date);
...
function getDates(displayMonths, maxDate, props, dateLib2) {
  ...
  const { addDays, differenceInCalendarDays, differenceInCalendarMonths, endOfBroadcastWeek, endOfISOWeek, endOfMonth, endOfWeek, isAfter, startOfBroadcastWeek, startOfISOWeek, startOfWeek } = dateLib2;
  const startWeekFirstDate = broadcastCalendar ? startOfBroadcastWeek(firstMonth, dateLib2) : ISOWeek ? startOfISOWeek(firstMonth) : startOfWeek(firstMonth);
...
function getInitialMonth(props, navStart, navEnd, dateLib2) {
  ...
  const { differenceInCalendarMonths, addMonths, startOfMonth } = dateLib2;
...
function getMonths(displayMonths, dates, props, dateLib2) {
  const { addDays, endOfBroadcastWeek, endOfISOWeek, endOfMonth, endOfWeek, getISOWeek, getWeek, startOfBroadcastWeek, startOfISOWeek, startOfWeek } = dateLib2;
  const dayPickerMonths = displayMonths.reduce((months, month) => {
    const firstDateOfFirstWeek = props.broadcastCalendar ? startOfBroadcastWeek(month, dateLib2) : props.ISOWeek ? startOfISOWeek(month) : startOfWeek(month);
...
function getNavMonths(props, dateLib2) {
  ...
  const { startOfYear, startOfDay, startOfMonth, endOfMonth, addYears, endOfYear, newDate, today } = dateLib2;
...
function getNextMonth(firstDisplayedMonth, calendarEndMonth, options, dateLib2) {
  ...
  const { startOfMonth, addMonths, differenceInCalendarMonths } = dateLib2;
...
function useCalendar(props, dateLib2) {
  ...
  const { startOfMonth, endOfMonth } = dateLib2;
...
function getFocusableDate(moveBy, moveDir, refDate, navStart, navEnd, props, dateLib2) {
  ...
  const { addDays, addMonths, addWeeks, addYears, endOfBroadcastWeek, endOfISOWeek, endOfWeek, max, min, startOfBroadcastWeek, startOfISOWeek, startOfWeek } = dateLib2;
  const moveFns = {
    day: addDays,
    week: addWeeks,
    month: addMonths,
    year: addYears,
    startOfWeek: (date) => broadcastCalendar ? startOfBroadcastWeek(date, dateLib2) : ISOWeek ? startOfISOWeek(date) : startOfWeek(date),
...
function addToRange(date, initialRange, min = 0, max = 0, required = false, dateLib2 = defaultDateLib) {
  const { from, to } = initialRange || {};
  const { isSameDay, isAfter, isBefore } = dateLib2;
  let range;
  if (!from && !to) {
...
function useRange(props, dateLib2) {
  ...
  const select = (triggerDate, modifiers, e) => {
    const { min, max } = props;
    const newRange = triggerDate ? addToRange(triggerDate, selected, min, max, required, dateLib2) : void 0;
